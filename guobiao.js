...
function calcfan_guobiao(tiles, seat, zimo) {
    ...
    function calc() {
        ...
        function calc0(tingpaifu) {
            ...
            // 6 番
            if (kezi_num === 4)
                ans.fans.push({'val': 6, 'id': 8028}); // 碰碰和
            if (hunyise && !qingyise)
                ans.fans.push({'val': 6, 'id': 8029}); // 混一色
            ...
            if (sansesanbugao)
                ans.fans.push({'val': 6, 'id': 8030}); // 三色三步高
            if (wumenqi)
                ans.fans.push({'val': 6, 'id': 8031}); // 五门齐
            let quanqiuren = true;
            if (zimo)
                quanqiuren = false;
            ...
            if (ans.fans.length === 0)
                ans.fans.push({'val': 8, 'id': 8040}); // 无番和
            // ---------------------------
            // ---------------------------
            // ---------------------------
            // ---------------------------
    ...
    if (result === 4) { // 一定是全不靠或七星不靠
        ...
        let ans = [];
        if (qixingbukao)
            ans.push({'val': 24, 'id': 8057}); // 七星不靠
        else
            ans.push({'val': 12, 'id': 8045}); // 全不靠
        updateret(ans);
    }
    return ret;
}
